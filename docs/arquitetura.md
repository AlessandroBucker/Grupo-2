...
## Frontend
O frontend será estruturado para organizar a interface do usuário de forma modular. O projeto adota o padrão MVC (Model-View-Controller) no front-end React, com uma camada de Service para comunicação com a API e Context API para estado global.

As principais pastas são:

- **components** – Componentes reutilizáveis da interface - Componentes reutilizáveis (Botões, Cards de KPI, Modais de confirmação).
- **pages** – Páginas principais da aplicação - Contém as telas completas da plataforma (Dashboard, Upload, Listagem).
- **services** – Responsável pela comunicação com o backend - Camada de comunicação ApiService.ts que faz as chamadas HTTP (Fetch) para o Backend Django.
